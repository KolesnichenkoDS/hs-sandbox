...
setLastSentMessage msg = modify (\c -> c { _lastSentMessage = Just msg })

setLastReceivedMessage :: MonadState PgContext m => PM.PgMessageB -> m ()
setLastReceivedMessage msg = modify (\c -> c { _lastReceivedMessage = Just msg })
        ...
        do
          ...
          isReady <- case msg of
            ...
            Just (PM.ReadyForQueryB _) -> do
              liftIO $ putStrLn "Ready for query"
              pure True
            Just _ -> pure False
            Nothing -> do
              pure False
